...
class WebSocketVoiceClient:
    ...
    def _register_functions(self):
        ...
        try:
            ...
            logger.info(f"Registered {len(self.available_functions)} functions from YAML config")
        except ImportError:
    ...
    async def _setup_session(self, connection):
        ...
        try:
            ...
            try:
                ...
                if not hasattr(session_updated, 'session') or session_updated.session is None:
    ...
    async def _handle_conversation_item_created(self, event, connection):
        ...
        try:
            ...
            if (hasattr(item, 'type') and 
                item.type == ItemType.FUNCTION_CALL and 
    ...
    async def _wait_for_event(self, connection, wanted_types: set, timeout_s: float = 10.0):
        ...
        try:
            ...
            logger.error(f"Timeout waiting for event types {wanted_types} after {timeout_s}s")
            raise
        except Exception as e:
    ...
    async def process_audio_input(self, audio_base64: str):
        ...
        if self.connection:
            await self.audio_processor.process_input_audio(audio_base64, self.connection)
